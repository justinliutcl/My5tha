...
public class WaitPayOrderAdapter extends BaseAdapter {
    ...
    public WaitPayOrderAdapter(GoodsorderUser user, Context context, LodingUtil lodingUtil, AddressAdapter.OnadapterChangeCall callback) {
...
//            holder.showorder.setVisibility(View.VISIBLE);
        ...
        holder.logistic.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                JumpUtil.jumpWithValue(context, LogisticActivity.class,new String[]{"orderNumber"},new String[]{mlist.get(position).getSubOrderList().get(0).getOrderNumber()},true);
            }
        });
