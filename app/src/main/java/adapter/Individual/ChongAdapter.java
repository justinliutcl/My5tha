...
public class ChongAdapter extends BaseAdapter{
    ...
    public ChongAdapter(Chonguser user, Context context) {
        this.user = user;
        this.context = context;
    }
    public void setUser(Chonguser users){
        user=users;
    }
    @Override
    public int getCount() {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.his.setText(user.getList().get(position).getPayType());
        holder.gwb.setText("+"+user.getList().get(position).getAmount());
        holder.type.setText("充");
        return convertView;
    }
